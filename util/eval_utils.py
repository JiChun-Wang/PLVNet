...
def compute_3d_iou_new(RT_1, RT_2, scales_1, scales_2, class_name_1, class_name_2):
    ...
    if (class_name_1 in ['bottle', 'bowl', 'can'] and class_name_1 == class_name_2):
        ...
        max_iou = 0
        for i in range(n):
            rotated_RT_1 = RT_1@z_rotation_matrix(2*math.pi*i/float(n))
            max_iou = max(max_iou,
...
def compute_RT_degree_cm_symmetry(RT_1, RT_2, class_id, synset_names):
    '''
    ...
    elif synset_names[class_id] in ['hmn']:
...
def compute_3d_matches(gt_class_ids, gt_RTs, gt_scales, synset_names,
                       pred_class_ids, pred_scores, pred_RTs, pred_scales,
...
def compute_RT_overlaps(gt_class_ids, gt_RTs,
...
def compute_degree_cm_mAP(final_results, synset_names, log_dir, degree_thresholds=[360], shift_thresholds=[100], iou_3d_thresholds=[0.1], iou_pose_thres=0.1, use_matches_for_pose=False, eval_recon=False, plot_figure=True):
    ...
    for result in progress:
        ...
        for cls_id in range(1, num_classes):
            ...
            iou_cls_gt_match, iou_cls_pred_match, _, iou_pred_indices = compute_3d_matches(cls_gt_class_ids, cls_gt_RTs, cls_gt_scales, synset_names,
                                                                                           cls_pred_class_ids, cls_pred_scores, cls_pred_RTs, cls_pred_scales,
                                                                                           iou_thres_list)
            ...
            RT_overlaps = compute_RT_overlaps(cls_gt_class_ids, cls_gt_RTs,
    ...
    kind_result["5 degree, 1 cm"] = "{:.1f}".format(
        pose_aps[-1, degree_thres_list.index(5), shift_thres_list.index(1)] * 100)
    kind_result["10 degree, 1 cm"] = "{:.1f}".format(
        pose_aps[-1, degree_thres_list.index(10), shift_thres_list.index(1)] * 100)
    kind_result["10 degree, 2 cm"] = "{:.1f}".format(
