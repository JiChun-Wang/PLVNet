...
class STN3d(nn.Module):
	...
	def forward(self, x):
		...
		iden = torch.tensor(np.array([1, 0, 0, 0, 1, 0, 0, 0, 1]).astype(np.float32)).view(1, 9).repeat(
...
class STNkd(nn.Module):
	...
	def forward(self, x):
		...
		iden = torch.tensor(np.eye(self.k).flatten().astype(np.float32)).view(1, self.k * self.k).repeat(batchsize,
		                                                                                                 1).to(
			x.device)
		x = x + iden
