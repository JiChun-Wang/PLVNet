...
def add_gravity_to_ob(body_id, link_id=-1, gravity=-10):
	...
	p.applyExternalForce(body_id, link_id, forceObj=[0, 0, gravity * ob_mass], posObj=[0, 0, 0],
						 flags=p.LINK_FRAME)
...
def create_urdf_for_mesh(mesh_dir, concave=False, out_dir=None, mass=0.1, has_collision=True, scale=np.ones((3))):
	...
	if concave:
		concave_str = 'yes'
		if mass != 0:
			collision_mesh_dir = mesh_dir.replace('.obj','_vhacd2.obj')
...
def creat_urdf_for_mesh_v2(obj_path):
	urdf_path = obj_path.replace('.obj', '.urdf')
	if not os.path.exists(urdf_path):
		urdf_str = f"""
			<robot name="model.urdf"> \n
				\t this is an urdf file. \n
			</robot> \n
			"""

		out_dir = obj_path.replace('.obj', '.urdf')
		with open(out_dir, 'w') as ff :
			ff.write(urdf_str)
...
def create_object(obj_file, scale, ob_in_world, mass, has_collision=True, useFixedBase=False, concave=False,
				  collision_margin=0.0001):
	'''
	@scale: np array (3)
	'''
	# urdf_dir = f'/tmp/{os.path.basename(obj_file)}_{uuid4()}.urdf'
	urdf_dir = obj_file.replace('.obj', '.urdf')
	create_urdf_for_mesh(obj_file, out_dir=urdf_dir, mass=mass, has_collision=has_collision, concave=concave,
						 scale=scale)
	q_wxyz = quaternion_from_matrix(ob_in_world)
	q_xyzw = [q_wxyz[1], q_wxyz[2], q_wxyz[3], q_wxyz[0]]
	ob_id = p.loadURDF(urdf_dir, basePosition=ob_in_world[:3, 3], baseOrientation=q_xyzw,
...
def create_duplicate_object(n_ob, obj_file, scale, ob_in_worlds, mass, has_collision=True, concave=False,
							useFixedBase=False, collision_margin=0.0001):
	ob_ids = []
	urdf_dir = None
	for i in range(n_ob):
		...
		q_xyzw = [q_wxyz[1], q_wxyz[2], q_wxyz[3], q_wxyz[0]]
		if i == 0:
			ob_id, urdf_dir = create_object(obj_file, scale=scale, ob_in_world=ob_in_world, mass=mass,
											has_collision=has_collision, concave=concave, useFixedBase=useFixedBase)
		else:
			ob_id = p.loadURDF(urdf_dir, basePosition=ob_in_world[:3, 3], baseOrientation=q_xyzw,
							   useFixedBase=useFixedBase,
							   flags=p.URDF_ENABLE_CACHED_GRAPHICS_SHAPES | p.URDF_USE_MATERIAL_COLORS_FROM_MTL | p.URDF_MAINTAIN_LINK_ORDER)
...
if __name__ == '__main__':
	obj_path = './bin.obj'
	creat_urdf_for_mesh_v2(obj_path)
